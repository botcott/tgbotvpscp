...
check_integrity() {
    ...
    if [ "$DEPLOY_MODE_FROM_ENV" == "docker" ]; then
        ...
        if [ -z "$bot_container_name" ]; then
            bot_container_name="tg-bot-${INSTALL_MODE_FROM_ENV}"
        fi
...
ask_env_details() {
    ...
    echo ""
    msg_info "Node-Agent Server Settings:"
    ...
    msg_info "Selected port: ${WEB_PORT}"
...
check_docker_deps() {
    ...
    run_with_spinner "Starting Docker service" sudo systemctl restart docker
...
install_systemd_logic() { 
    ...
    local ip=$(curl -s --connect-timeout 5 ipinfo.io/ip || echo "Could not determine"); echo ""; echo "---"; msg_success "Installation (Systemd) complete!"; msg_info "IP: ${ip}:${WEB_PORT}"; echo "---";
}
...
create_docker_compose_yml() {
    msg_info "Creating docker-compose.yml (forwarding port ${WEB_PORT})..."
...
update_bot() {
    ...
    if [ "$DEPLOY_MODE_FROM_ENV" == "docker" ]; then
        ...
        # create_docker_compose_yml now uses WEB_PORT
        create_docker_compose_yml 
